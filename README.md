...
- [`rust`](profiles/bundles/rust/README.md): A Rust repository: toolchain,
  ...
  `devset`. Features: `docs`, `agents`, `publish`, `binaries`, `nightly`,
  `strict`.
...
| [`cargo-deny`](profiles/cargo/cargo-deny/README.md)           | cargo-deny: yanked and unmaintained crates, one version each, permissive licences, crates.io only, and the house bans                           | `deny.toml`, `.config/mise/mise.lock` (keys)                                                                                                       |
...
| Profile                                                            | What                                                                                                        | Owns                            |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- | ------------------------------- |
| [`devset`](profiles/devset/devset/README.md)                       | devset itself: pinned for every machine and job, and drift from the profiles failing the checks             | `.config/mise/mise.lock` (keys) |
| [`devset-collection`](profiles/devset/devset-collection/README.md) | For a collection of profiles: its catalog, the rules its profiles keep, its test suite, and its pins locked |                                 |
...
| [`git-changelog`](profiles/git/git-changelog/README.md)   | git-cliff writes the changelog from Conventional Commits at each release, grouped, linked, breaking marked | `cliff.toml`, `.config/mise/mise.lock` (keys)     |
...
| [`github-ci`](profiles/github/github-ci/README.md)                       | GitHub Actions: every `check-*` recipe a job of its own, read from the justfile, tools from the mise lock; a Pages site built once and deployed | `.github/workflows/check.yml`                                                                                                                               |
...
| [`github-release`](profiles/github/github-release/README.md)             | A release from its tag: every package-* recipe on each platform, then the GitHub Release with git-cliff's notes                                 | `.github/workflows/release.yml`                                                                                                                             |
...
| [`rust-doc`](profiles/rust/rust-doc/README.md)             | rustdoc builds every crate's documentation, private items included, warnings denied                                                  | `.claude/` (10 files)        |
| [`rust-fmt`](profiles/rust/rust-fmt/README.md)             | rustfmt on nightly options: imports by module, comments wrapped, doc examples formatted                                              | `rustfmt.toml`               |
...
| `authors`       | empty               | Authors, comma-separated                                        | [`cargo-workspace`](profiles/cargo/cargo-workspace/README.md)                                                                                                                                                                             |
...
| `license`       | `MIT OR Apache-2.0` | The licence, an SPDX expression                                 | [`cargo-workspace`](profiles/cargo/cargo-workspace/README.md)                                                                                                                                                                             |
...
| `name`          | empty               | The first crate's name; empty takes the directory's             | [`cargo-workspace`](profiles/cargo/cargo-workspace/README.md)                                                                                                                                                                             |
| `repository`    | none                | The GitHub repository, owner/name                               | [`cargo-workspace`](profiles/cargo/cargo-workspace/README.md), [`git-changelog`](profiles/git/git-changelog/README.md)                                                                                                                    |
