#!/usr/bin/env python3
"""Moves the house's nightly to the newest one of the last two weeks that has every component.

Usage: scripts/rust-nightly.py <rust-toolchain.toml> [<report>]

A nightly qualifies when its manifest has every component the file names available for every
platform the house locks: rustup builds some components only on some nights. The channel never
