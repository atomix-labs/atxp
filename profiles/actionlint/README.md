...
expressions, and, where [`shellcheck`](../shellcheck/README.md) is applied too,
the shell in each `run:` step. It checks correctness;
[`zizmor`](../zizmor/README.md) checks security.
...
| `.just/actionlint.just`                      | whole | owned  |          |
| `.config/mise/conf.d/devset-actionlint.toml` | whole | owned  |          |
| `.config/mise/mise.lock`                     | keys  | owned  |          |

## Recipes

- `check-actionlint`: Checks every workflow: syntax, expressions, and the shell
  in each step.
