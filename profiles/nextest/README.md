...
| `.just/nextest.just`                      | whole | owned  |       |
| `.config/mise/conf.d/devset-nextest.toml` | whole | owned  |       |
| `.config/mise/mise.lock`                  | keys  | owned  |       |

## Recipes

- `check-nextest`: Runs every test, then the doctests nextest leaves to cargo;
