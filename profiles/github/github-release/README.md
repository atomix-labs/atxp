...
[`cargo-binaries`](../../cargo/cargo-binaries/README.md)' for one, it runs `just
package` on linux-x64, linux-arm64 and macos-arm64, the platforms the locks
cover; then it attests every archive and publishes the GitHub Release,
git-cliff's notes for the tag and every archive attached. Without one, the
Release has the notes alone. Anyone can check an archive's build provenance with
`gh attestation verify <archive> --repo <owner>/<repository>`.
...
revoked when the job ends, so the repository stores none. That job runs in the
