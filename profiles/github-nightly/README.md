...
the tools mise locks: the checks too slow for every change, such as
[`cargo-hack`](../cargo-hack/README.md) over each feature, or links to the web
with [`lychee`](../lychee/README.md). A night with no commit since the last
green one is skipped, as [`automation`](../automation/README.md) sets; a run by
hand always runs.
