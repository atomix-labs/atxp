"""A profile collection's pins: each tool's version, its lock entry for every platform, and the
dprint plugins its payloads name.

Usage: profile-pins.py check | lock [<profile>...] | bump [<report>]

Run in a collection's root, which holds `profiles/<id>/` and applies the `mise` atom. `check`,
...
def helper():
    """The mise atom's helper, applied to this collection, whose checks every entry is held to."""
...
def github(url):
    ...
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "devset-profiles"}
...
def plugin_file(path, moved, held):
    ...
    for match in list(PLUGIN.finditer(text)):
        ...
        if wasm and (wasm.get("digest") or "").startswith("sha256:"):
            ...
            request = urllib.request.Request(new, headers={"User-Agent": "devset-profiles"})
